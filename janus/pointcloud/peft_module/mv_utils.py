...
def points2pos(points, length, size_x=4, size_y=4, pos_cor=[-1.0, 0.0, 1.0]):
    """
    :param points: [B, num_points, 3]
    :param size_x:
    :param size_y:
    ...
    direction = torch.tensor(pos_cor, requires_grad=False, device=points.device)
    ...
    min_projections = torch.min(projections, dim=1)[0].unsqueeze(1).repeat([1, projections.size(1)])
    max_projections = torch.max(projections, dim=1)[0].unsqueeze(1).repeat([1, projections.size(1)])

    normalized_projections = (projections - min_projections) / (max_projections - min_projections)
    final_projections = normalized_projections * length

    _i = torch.linspace(-size_x / 2, (size_x / 2) - 1, size_x, requires_grad=False, device=final_projections.device)
    final_projections = final_projections + _i  # [batch, num_points, size_x]
...
class PCViews:
    ...
    def get_pos(self, points, pos_cor):
    ...
    @staticmethod
    def point_transform(points, rot_mat, translation):#rot_mat2,
        """
