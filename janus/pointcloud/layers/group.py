...
class GroupingOperation(Function):
    ...
    def forward(ctx, features: torch.Tensor, idx: torch.Tensor) -> torch.Tensor:
        ...
        # Expand indices for gathering: (B, 1, npoint, nsample) -> (B, C, npoint, nsample)
        idx_expanded = idx.unsqueeze(1).expand(-1, C, -1, -1)
        
        # Gather features using advanced indexing
        output = torch.gather(features.unsqueeze(2).expand(-1, -1, npoint, -1), 
                             dim=3, 
                             index=idx_expanded)
    ...
    def backward(ctx, grad_out: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        """
        :param ctx: 
        :param grad_out: (B, C, npoint, nsample) tensor of gradients
        :return: (B, C, N) gradient of features
        """
        ...
        grad_features.scatter_add_(2, 
                                 idx_expanded.reshape(B, C, -1), 
                                 grad_out.reshape(B, C, -1))
...
class GatherOperation(Function):
    ...
    def forward(ctx, features: torch.Tensor, idx: torch.Tensor) -> torch.Tensor:
        ...
        :param ctx: context for backward pass
        :param features: (B, C, N) input features
        :param idx: (B, npoint) indices to gather
        :return: (B, C, npoint) gathered features
        """
        assert features.is_contiguous()
        assert idx.is_contiguous()
        ...
        # Create expanded index (B, C, npoint)
...
class BallQuery(Function):
    ...
    def forward(ctx, radius: float, nsample: int, xyz: torch.Tensor, new_xyz: torch.Tensor) -> torch.Tensor:
        """
        Pure PyTorch implementation of ball query
        :param ctx: context for backward pass
        :param radius: float, search radius
        :param nsample: int, maximum number of neighbors
        :param xyz: (B, N, 3) all points
        :param new_xyz: (B, npoint, 3) query points
        :return: (B, npoint, nsample) neighbor indices
        """
        ...
        # Compute pairwise distances (B, npoint, N)
        diff = new_xyz.unsqueeze(2) - xyz.unsqueeze(1)  # (B, npoint, N, 3)
        dist = torch.sum(diff ** 2, dim=-1)  # (B, npoint, N)

        # Initialize output index tensor
        idx = torch.zeros((B, npoint, nsample), dtype=torch.long, device=device)

        for b in range(B):
            for i in range(npoint):
                # Find points within radius
                mask = dist[b, i] <= radius * radius
                valid_indices = torch.nonzero(mask, as_tuple=True)[0]

                # Random sample if too many points
                if len(valid_indices) > nsample:
                    valid_indices = valid_indices[torch.randperm(len(valid_indices))[:nsample]]
                elif len(valid_indices) < nsample:
                    # Pad with first point if not enough points
                    pad_size = nsample - len(valid_indices)
                    valid_indices = torch.cat([valid_indices, valid_indices[:1].repeat(pad_size)])

                idx[b, i] = valid_indices
