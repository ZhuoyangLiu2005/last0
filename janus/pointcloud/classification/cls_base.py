...
class BaseCls(nn.Module):
    ...
    def forward(self, data, args, target): 
        global_feat = self.encoder.forward_cls_feat(data, args=args, target=target)
        return self.prediction(global_feat)

    def get_loss(self, pred, gt, inputs=None):
        return self.criterion(pred, gt.long())

    def get_logits_loss(self, data, gt, args):
        logits = self.forward(data, args, None)
